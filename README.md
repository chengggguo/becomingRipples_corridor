...
4. `arduino/legacy/190cmBar_device_no_led_only/190cmBar_device_no_led_only.ino`：无外部 RUN/RESET 的自主运行装置版；保留原始随机运动和按轮数自动重启逻辑，只移除了 LED 控制路径。
5. `arduino/tests/`：现场排查和短时间验证用的临时测试程序，不是当前正式上传文件。
...
6. `arduino/legacy/190cmBar_device_no_led_only/190cmBar_device_no_led_only.ino`：不包含外部 RUN/RESET 的自主随机运行、无 LED 版本。
...
6. `arduino/legacy/190cmBar_device_no_led_only/190cmBar_device_no_led_only.ino`: standalone random-motion version without external RUN/RESET inputs; the LED control path is removed.
