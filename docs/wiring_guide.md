...
## Bus Between Two Controllers / 两块控制器之间的 Bus
...
注意：这个 Bus 方案需要两块 group controller 共地。
Note: this bus option requires the two group controllers to share GND.

如果两块控制器之间的线很长，建议使用双绞线，并考虑在 Bus 线上加一个外部上拉电阻，例如 4.7kΩ 到 5V。
If the wire between controllers is long, use twisted pair if possible and consider adding an external pull-up resistor, such as 4.7kΩ to 5V, on the bus line.

## Relay Module Control Side / 继电器模块控制侧
...
## Relay Contact Side to Devices / 继电器触点侧到装置
...
### AUTOHOME/RESET Request Relays / AUTOHOME/RESET 请求继电器
...
## Recommended Protection Resistors / 推荐保护电阻
...
## Power and Ground Boundaries / 供电和地线边界
...
## Quick Test Order / 快速测试顺序

建议按以下顺序测试。
Recommended test order:

1. 单独测试一台装置的 `D2 RUN`：手动短接 `D2` 到 `GND`，确认装置运行。
   Test one device's `D2 RUN`: manually short `D2` to `GND` and confirm the device runs.
2. 单独测试一台装置的 `D3 AUTOHOME/RESET`：手动短接 `D3` 到 `GND`，确认装置在 IDLE 时归零。
   Test one device's `D3 AUTOHOME/RESET`: manually short `D3` to `GND` and confirm the device homes in IDLE.
3. 用继电器测试一台装置的 `RUN` 和 `RESET`。
   Test one device's `RUN` and `RESET` through relays.
4. 接上本排三台装置。
   Connect all three local devices.
5. 接上 LD2410C，确认 presence 能启动三台。
   Connect the LD2410C and confirm presence starts all three devices.
6. 接上两块 group controller 的 `D3/GND` Bus，确认任意一侧触发时六台都会运行。
   Connect the `D3/GND` bus between the two group controllers and confirm either side can start all six devices.
