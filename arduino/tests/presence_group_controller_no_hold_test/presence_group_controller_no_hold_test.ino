/*
  Becoming Ripples - Presence Group Controller temporary no-hold test
  Board: Arduino Nano R3 / Uno compatible

  This is a temporary test variant of:
  arduino/controllers/presence_group_controller/presence_group_controller.ino

  Difference from the main controller sketch:
  - Local presence is debounced for 500ms, but it is not held for 3 minutes.
  - D13 and RUN relays follow current local presence or remote bus request.
  - Reset scheduling and relay pin behavior are kept the same.
*/
...
void loop() {
  ...
  bool localRunRequest = readDebouncedLocalPresence(now);
