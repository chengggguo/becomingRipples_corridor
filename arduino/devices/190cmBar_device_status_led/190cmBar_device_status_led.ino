...
DeviceStatus deviceStatus = DEVICE_STATUS_IDLE;
...
void setDeviceStatus(DeviceStatus newStatus) {
    ...
    lastStatusLedToggleTime = millis();
    ...
    digitalWrite(statusLedPin, statusLedState ? HIGH : LOW);
}

void updateStatusLed() {
    if (deviceStatus != DEVICE_STATUS_AUTOHOME) {
        return;
    }

    unsigned long now = millis();
    if (now - lastStatusLedToggleTime >= statusLedBlinkIntervalMs) {
        lastStatusLedToggleTime = now;
...
void setup() {
    ...
    setDeviceStatus(DEVICE_STATUS_AUTOHOME);
...
void loop() {
  ...
  if (readRunSignal()) {
    if (!wasRunning) {
      setDeviceStatus(DEVICE_STATUS_RUN);
