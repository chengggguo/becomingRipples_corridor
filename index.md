...
- `docs/communication_model.md`
  ...
  Hardware and signal model between sensor, group controller, relays, and device Arduinos.

- `docs/bus_options.md`
  两个控制器之间共享房间 presence 状态的 Bus 方案。
  Cross-controller bus options for sharing room-level presence.
...
- `arduino/presence_group_controller/`
  ...
  New firmware for each row-level presence controller.
